<section class="space-y-6">
    <header>
        <h4 class="text-lg font-medium text-red-900">
            {{ __('Xóa tài khoản') }}
        </h4>
    ...
    <x-danger-button style="background: red;"
        x-data=""
    ...
    <x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{ route('profile.destroy') }}" class="p-6">
            ...
            <h2 class="text-lg font-medium text-gray-900">
                {{ __('Bạn có chắc chắn xóa tài khoản vĩnh viễn?') }}
            </h2>
            ...
            <div class="mt-6 flex justify-end">
                <x-secondary-button style="background: green;" x-on:click="$dispatch('close')">
                    {{ __('Hủy') }}
                </x-secondary-button>

                <x-danger-button style="background: red;" class="ml-3">
                    {{ __('Xóa tài khoản') }}
                </x-danger-button>
            </div>
        </form>
